...
## v1.26.0 — בורר איכות אמיתי ("שאל אותי") עם דיאלוג בחירה

### הבעיה לפני

ב-v1.25 נוסף בורר איכות עם שלוש אפשרויות, אבל **"שאל אותי" התנהג בדיוק כמו "הטובה ביותר"** — לא הייתה שום שאלה בפועל. המשתמש ביקש שבאמת ישאל אותו איזו רזולוציה הוא רוצה.

### הפתרון

הזרימה של 🎥 פוצלה לשני שלבים נפרדים כדי לאפשר שאלה באמצע **בלי לחלץ פעמיים** (הקישורים החתומים פגים אחרי ~שעתיים, וחילוץ חוזר היה מכפיל את ההמתנה):

1. **שלב איתור** (`resolveRecordingCandidates`) — פותח כל הקלטה בטאב נסתר, לוחץ Play, ואוסף את **כל** הקישורים החתומים (כל הרזולוציות הזמינות). רץ במקביל כמו קודם.
2. **דיאלוג בחירה** — `summariseResolutions` בונה את רשימת הרזולוציות הייחודיות (גבוה→נמוך, עם כמה הקלטות מציעות כל אחת), ומודאל בפופאפ (`chooseQualityDialog`) מציג אותן ככפתורים. המשתמש בוחר אחת.
3. **שלב הורדה** (`downloadResolvedRecordings`) — מוריד מהקישורים **שכבר נאספו** בשלב 1, לפי הבחירה.

### דקויות

- אם להקלטה מסוימת אין בדיוק את הרזולוציה שנבחרה — `pickRecordingUrlByLabel` בוחר את **הקרובה ביותר** (לפי ספירת פיקסלים), כי הקלטות שונות יכולות להציע רזולוציות שונות.
- אם בכלל יש רק רזולוציה אחת זמינה — אין מה לשאול, יורד אוטומטית עם הודעה מתאימה.
- ביטול (כפתור "ביטול" / Esc / לחיצה על הרקע) עוצר בלי להוריד כלום.
- "הטובה ביותר" ו"הקטנה ביותר" ממשיכים לעבוד בדיוק כמו קודם (זרימה לא-אינטראקטיבית; `downloadZoomRecordings` הפך ל-wrapper דק מעל שני השלבים).

### איך לבחון

1. דף Zoom של אריאל → סרוק → בחר הקלטה אחת או יותר
2. בבורר האיכות בחר **"שאל אותי (בחר איכות)"**
3. לחץ **🎥 הורד סרטונים** → המתן לשלב האיתור (~25 שניות לכל הקלטה)
4. אמור לקפוץ מודאל עם רשימת הרזולוציות → בחר אחת
5. ההורדה מתחילה עם הרזולוציה שנבחרה. אפשר לסגור את הפופאפ — ממשיך ברקע.

> אם קופצת הודעה "רק איכות אחת זמינה" — זה תקין; להקלטות של אריאל לרוב יש מקור MP4 בודד. הדיאלוג מופיע רק כשבאמת יש יותר מרזולוציה אחת.
