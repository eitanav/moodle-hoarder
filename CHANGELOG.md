...
## v1.34.1 — תיקון RTL/BiDi בפופאפ (השלמת ההכנה ל-v2)

**הבעיה:** בפופאפ, טקסטים שמערבבים עברית עם אנגלית / כתובות / `<code>` נשברו ע"י אלגוריתם ה-BiDi — שורות שמתחילות מהצד הלא נכון, כתובות בסוגריים שמתהפכות (`(course/view.php)`), ו-`chrome.debugger` שמופיע הפוך בתוך משפט עברי. ה-popup פיגר אחרי `options.html` שכבר טופל.

**התיקון:**
- CSS: `unicode-bidi: plaintext` ל-`.hint`, `#status` ו-`#log` (כל פסקה/שורה דינמית מחליטה על כיוונה בעצמה); בידוד גלובלי ל-`<code>` (`direction: ltr; unicode-bidi: isolate`) — כך popup.html תואם עכשיו לסטנדרט של options.html.
- שכתוב ה-hints הבעייתיים שיתחילו בעברית ובלי כתובות בסוגריים: hint של Zoom, hint הדיאגנוסטיקה, ותווית ה-network debug (תורגמה לעברית מלאה).
- **תיקון באג סמוי:** ה-hints `initial.hint` ו-`deadlines.help.ical` נושאים `data-i18n`, כך ש-i18n.js דורס אותם בזמן ריצה. הטקסט הפולישט ב-popup.html היה מתבטל בפועל כי i18n.js עדיין החזיק את הגרסה הישנה עם הכתובות בסוגריים. סונכרנו המחרוזות בעברית ב-i18n.js כדי שהתיקון יופיע באמת ב-UI החי.
