...
## v1.32.0 — 🎥 הורדת Zoom עובדת: fetch ל-blob במקום URL ישיר

הסאגה של ה-.htm נסגרת. v1.31.0 פיצח את **המנגנון** (Referer של דומיין החשבון), אבל ההורדה עדיין שמרה `.htm`. ה-trace השני הוכיח את החשד: **`declarativeNetRequest modifyHeaders` לא חל על בקשות `chrome.downloads.download`** — הן browser-initiated, וכלל ה-Referer לא נוגע בהן. אז ה-CDN קיבל בקשה בלי Referer → 403 HTML → קובץ `.htm` "File wasn't available on site".

### הראיה

ה-`download-probe` ב-🩺 דיאגנוסטיקה כבר הראה את ההבדל: **fetch מהפופאפ** (שהוא `xmlhttprequest`, ש-DNR **כן** משנה) החזיר `video/mp4` אמיתי עם הכלל פעיל, בעוד `chrome.downloads.download` על אותו URL שמר HTML. אותו URL, אותו כלל — ההבדל הוא רק סוג הבקשה.

### הפתרון

`downloadResolvedRecordings` כבר לא מעביר את ה-URL החתום ישירות ל-`chrome.downloads`. במקום זה:

1. **`fetchRecordingBlob(url)`** — מביא את ה-MP4 ב-`fetch` מהפופאפ. כי זו בקשת `xmlhttprequest`, כלל ה-Referer חל → ה-CDN מגיש את הווידאו האמיתי. קורא את הגוף ב-streaming reader ומדווח התקדמות בייטים (`45.2MB / 115.0MB`).
2. בדיקות הגנה: status שאינו 200, `content-type: text/html`, או גוף ריק → שגיאה ברורה ("הקישור כנראה פג, סרוק מחדש") במקום לשמור `.htm`.
3. **`downloadBlobAndRevoke(blob, filename)`** — שומר את ה-blob דרך `chrome.downloads.download` עם `URL.createObjectURL`, ומשחרר את ה-URL רק כש-`onChanged` מדווח `complete`/`interrupted` (עם safety-net של 15 דק').

### מה השתנה ב-UX

- מד התקדמות אמיתי בזמן ההורדה: `🎥 שם — מוריד 45.2MB / 115.0MB`.
- ההודעה הסופית מציינת גודל כולל: `🎥 3/3 הקלטות ירדו (340.5MB) ✅`.
- **הודעה חדשה: "אל תסגור את הפופאפ עד שההורדה תיגמר".** ה-blob חי בפופאפ, אז סגירה באמצע הכתיבה תקטע אותה. (שלב ה-resolve ממילא דרש פופאפ פתוח, אז זה לא רגרסיה אמיתית.)

### מגבלה ידועה

הקובץ כולו נשמר בזיכרון לפני הכתיבה לדיסק. ל-~115MB (הגודל הטיפוסי) זה עובד חלק. קבצים של כמה GB עלולים להעיק על הזיכרון — לעתיד: streaming לדיסק עם File System Access API. ראה SESSION-NEXT.

### איך לבחון

1. `update.bat` (או `git pull` + Reload התוסף — אין הרשאות חדשות, אבל Reload חובה).
2. סרוק קורס עם הקלטת Zoom → בחר הקלטה → **🎥 הורד סרטונים**.
3. צפוי: מד התקדמות בייטים, ואז קובץ **`.mp4` תקין שמתנגן** (לא `.htm`).
4. אם נכשל עם "הקישור כנראה פג" — סרוק מחדש סמוך ללחיצה (ה-URLs החתומים פגים תוך ~שעתיים).
