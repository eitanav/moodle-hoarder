...
## v2.9.6 — זום: הגנה מהורדת אותו וידאו כמה פעמים + אבחון

**הרקע:** דווח שבבחירת כמה הקלטות יורד **אותו וידאו עם שמות שונים**. בדיקת
ההיסטוריה מראה ש-`clickRecordingRow`/`clickPlayAndCaptureUrl`/`navigateToZoomPage`
**לא שונו מאז v1.31.0** — כלומר הקוד שלנו לא הרגרסיב; ככל הנראה Zoom שינו את דף
ההקלטות, ולכן לחיצה על שורה כבר לא פותחת דף פרטים נפרד לכל הקלטה והחילוץ מחזיר
את אותו קישור.

- **`popup.js`** (handler של 🎥, לא בקוד ה-resolver הרגיש) — אם החילוץ מחזיר את
  אותו קישור share לכמה הקלטות, מסירים כפילויות לפי URL כדי לא להוריד את אותו
  קובץ מספר פעמים, ומציגים אזהרה ברורה שמסבירה מה קרה ומבקשת את קובץ
  `zoom-detail-debug` לתיקון מדויק. שינוי אדיטיבי בלבד — כשהקישורים שונים (המקרה
  התקין) ההתנהגות זהה לחלוטין.

> תיקון השורש (שכל הקלטה תיפתח לדף הפרטים שלה ב-Zoom החדש) דורש לראות את ה-DOM
> הנוכחי של Zoom — לכן נחוץ קובץ ה-debug.

---

## v2.9.5 — תיקון: הורדת כמה הקלטות זום עצרה על "1 מתוך 1"

**הבעיה:** בבחירת כמה הקלטות זום והורדת הסרטונים, רק ההקלטה הראשונה ירדה
("מוריד 1 מתוך 1"). הסיבה: רשימת ההורדה נבנית רק מהקלטות שחולץ להן קישור share
(`withUrls`), ואחרי שההקלטה הראשונה נפתחה וחזרנו לרשימה, ה-SPA של Zoom עדיין היה
באמצע מעבר — אז הלחיצה על השורה הבאה נכשלה בשקט וכל שאר ההקלטות נשמטו.

- **`popup.js`** — `resolveZoomPlayUrls` הוקשח בצורה **אדיטיבית** בלבד: לפני כל
  שורה מוודאים שאנחנו על רשימת ההקלטות (`ensureOnZoomList`, helper חדש שמשתמש רק
  בפונקציות הקיימות), ואם לחיצת השורה לא נתפסה או שדף הפרטים לא נטען — מתאוששים
  ומנסים את אותה הקלטה פעם נוספת לפני שמוותרים. מסלול ההצלחה הקיים לא השתנה
  (כשהרשימה כבר מוצגת הבדיקה חוזרת תוך ~100ms), ושום פונקציית resolver רגישה לא
  נגעו בה.

---

## v2.9.4 — תמלול: ריפוי-עצמי כשמריצים תחת Python בלי התלויות

**הבעיה:** אם מפעילים את `run_web.py` עם Python כלשהו שב-PATH (או דרך שיוך
קובץ/סוכן) במקום ה-`.venv` הייעודי, חסרים בו `faster-whisper`/`huggingface_hub`
והתמלול נופל באמצע עם "Missing dependency huggingface_hub".

- **`server.py`** — נוספה `ensure_dependencies()` שרצה בהפעלה: אם התלויות חסרות
  ב-Python הנוכחי אבל קיים `transcriber/.venv` שיש בו אותן, השרת **מפעיל את עצמו
  מחדש אוטומטית** דרך אותו `.venv`. אם אין `.venv` מתאים — מודפסת הודעה ברורה עם
  פקודת ההתקנה המדויקת ל-Python הזה, וה-UI עדיין נטען.
- **`model_manager.py` / `engine.py`** — הודעות התלות החסרה מפנות עכשיו ל-
  `run_web_windows.bat` (בנוסף ל-GUI), ומדגישות שההתקנה היא לסביבה הנוכחית.
- **בדיקות** — נוספו בדיקות ל-`ensure_dependencies`/`_core_deps_present`.

---

## v2.9.3 — רידיזיין לאפליקציית התמלול המקומית: ממשק web בעיצוב התוסף

**הבעיה:** ממשק התמלול המקומי היה GUI ישן של Tkinter שאי אפשר באמת לעצב כמו
התוסף — בלי gradient, בלי toggles בסגנון iOS, בלי מצב כהה/בהיר אמיתי, ובלי דרך
נוחה לתמלל הרבה קבצים ברצף. ביקשת רידיזיין שמיש שיראה כמו התוסף.

**מה חדש — אפליקציית web מקומית (`transcriber/`):** שרת מקומי קטן (ספריית התקן
של Python בלבד, בלי תלויות חדשות) שמגיש ממשק דפדפן באותו theme של התוסף. רץ
לגמרי על המחשב — שום הקלטה לא עוזבת אותו, והכל מאזין רק ל-`127.0.0.1`.

- **חדש `transcriber/mh_transcriber/server.py`** — שרת HTTP + API + תור עיבוד
  ברקע + SSE להתקדמות חיה + שמירת מצב (`~/.moodle-hoarder-transcriber/`).
- **חדש `transcriber/mh_transcriber/web/`** — `index.html` / `styles.css` /
  `app.js`. אותם משתני CSS, gradient ורוד→כתום, RTL, מצב **כהה/בהיר/מערכת**
  ו-accent **ורוד/כחול-אריאל** — בדיוק כמו `options.html`/`popup.html`.
- **מחסנים (warehouses)** — אוספי קבצים בעלי שם; כל מחסן הוא תור נפרד.
- **גרירה והעלאה** של קבצי מדיה, עם רשימת קבצים, badges של סטטוס ופס התקדמות
  לכל קובץ.
- **תמלול ברצף** של כל המחסן בלחיצה אחת, עם אפשרות עצירה באמצע (כולל ביטול
  באמצע קובץ).
- **בחירת מיקום פלט** גלובלי ולכל מחסן, ו**בחירת פורמטים** (TXT/SRT/VTT/JSON)
  שייווצרו לכל תמלול.
- **חדש `transcriber/run_web.py` + `transcriber/run_web_windows.bat`** — הפעלה
  בדאבל-קליק שפותחת את הממשק בדפדפן.
- **`engine.py` + `formatters.py`** — `write_outputs`/`transcribe_file` מקבלים
  עכשיו `formats` לבחירת קבצי הפלט (ברירת מחדל: כל הארבעה, תאימות לאחור).
- **בדיקות** — נוספו `tests/test_server.py` ובדיקות סינון פורמטים ב-
  `tests/test_formatters.py`.

> ה-GUI הישן (`run_gui.py`) וה-CLI (`transcribe.py`) ממשיכים לעבוד כרגיל; הממשק
> החדש הוא דרך נוספת ומומלצת על אותו מנוע תמלול.

---

## v2.9.2 — הצגת סיבת הכשל בשליחת פידבק

- **`options.js`** — כשהשליחה נכשלת, מוצגת כעת הסיבה האמיתית (הודעת השרת מ-Web3Forms,
  או שגיאת הרשת) במקום "נכשלה" כללי — מקל על אבחון (למשל "מייל לא אומת").

---

## v2.9.1 — הפעלת טופס הפידבק (מפתח Web3Forms)

- **`updates.js`** — הוגדר `MH_FEEDBACK_KEY` (מפתח Web3Forms אנונימי). טופס
  הפידבק בדף ההגדרות שולח מעכשיו ישירות למייל של המפתח. המייל לא בקוד.

---

## v2.9.0 — טופס פידבק בתוך התוסף (נשלח למייל בלי לחשוף אותו)

במקום קישור ל-GitHub Issues, נוסף **טופס פידבק מלא בדף ההגדרות**: בחירת סוג
(באג / הצעת פיצ׳ר / אחר), תיבת טקסט, ושדה מייל-לחזרה אופציונלי. הטופס נשלח דרך
**Web3Forms** ישירות למייל של המפתח — **המייל לא מופיע בקוד** (יש רק מפתח גישה
אנונימי; המייל יושב אצל Web3Forms). אם לא הוקדם מפתח או אם השליחה נכשלת, יש
נפילה חיננית לקישור GitHub Issues.

- **`updates.js`** — `MH_FEEDBACK_KEY` (ריק כברירת מחדל), `MH_FEEDBACK_ENDPOINT`,
  `MH_ISSUES_URL`.
- **`options.html`/`options.js`** — סקשן "💬 פידבק והצעות" עם טופס + `sendFeedback()`
  ששולח POST ל-Web3Forms (כולל גרסה ושפה לקונטקסט), עם סטטוס: שולח / נשלח / ריק /
  נכשל. מוסיף לכל הודעה את הגרסה הנוכחית.
- **`manifest.json`** — נוספה host permission ל-`api.web3forms.com`.
- **`i18n.js`** — כל מחרוזות הטופס בשתי השפות.
- **`PRIVACY.md`** — תיעוד: נשלח רק מה שאתה מקליד, ורק בלחיצה מפורשת על "שלח".
...
ל-~5 דקות, אז "בדוק עכשיו" קיבל לפעמים את הגרסה הישנה והציג "אתה מעודכן".

- **`updates.js`** — נוסף cache-buster (`?cb=<timestamp>`) ל-URL של בדיקת
  העדכון, כך שכל בדיקה כפויה קוראת גרסה טרייה ועוקפת את ה-CDN edge cache.

---

## v2.8.1 — bump לבדיקת העדכון בלחיצה מקצה-לקצה

bump גרסה בלבד, כדי לבדוק את כל זרימת ה-Native Host: גרסה מותקנת 2.8.0 (עם
ה-host מותקן) אמורה לזהות 2.8.1, וללחיצה על "⬇️ עדכן עכשיו" → "🔄 רענן עכשיו"
להעלות אותה ל-2.8.1 בלי לפתוח קבצים.
...
- bump גרסה כדי לבדוק את זרימת העדכון מקצה-לקצה: גרסה מותקנת 2.7.6 שתבדוק מול
  `main` שב-2.7.7 אמורה להציג את הבאנר "🆕 גרסה חדשה זמינה" עם כפתור "🔄 רענן עכשיו".
...
- **`updates.js`** — קבוע `MH_DONATE_URL` (ריק כברירת מחדל). מילוי הקישור של
  Buy Me a Coffee מפעיל את הכפתור; ריק = הכפתור מוסתר לחלוטין.
- **`options.html`** — כפתור `☕ קנה לי קפה / תודה` מתחת לכרטיס "כמה זמן חסכת"
  (נקודת ה-conversion הטבעית), בסגנון BMC; מוסתר עד שמוגדר קישור.
- **`options.js`** — מציג את הכפתור ומגדיר את ה-href רק כש-`MH_DONATE_URL` מוגדר.
- **`i18n.js`** — `opt.about.donate` בשתי השפות.
- **`ROADMAP-PRO.md`** — תועדו ההחלטות (חינם לעד, בלי פרסומות, תרומה ב-BMC) +
  סעיף חדש על שמירת מסמכי התכנון פרטיים כשהריפו נהיה ציבורי.

---

## v2.7.4 — תיעוד תנאי "ריפו ציבורי" + bump לבדיקת זרימת העדכון

- **`ROADMAP-PRO.md`** — נוסף ממצא חשוב: בדיקת העדכונים האוטומטית (וגם ההתקנה
  עצמה) דורשות שהריפו יהיה **ציבורי**; בריפו פרטי GitHub מחזיר 404 ו"בדוק עכשיו"
  מציג "נכשלה". נוסף גם צ'קליסט קצר לפני פרסום.
- bump גרסה כדי שאפשר יהיה לראות את באנר "גרסה חדשה זמינה" בפועל: גרסה מותקנת
  2.7.3 שתבדוק מול `main` שב-2.7.4 (לאחר שהריפו יהפוך לציבורי) תציג את הבאנר.

---

## v2.7.3 — קישור פידבק (GitHub Issues) בדף ההגדרות

- **`options.html`** — נוסף בסקשן "אודות" קישור "דיווח על באג או הצעה לשיפור"
  שפותח את עמוד ה-Issues של הפרויקט ב-GitHub. ערוץ פידבק ראשוני ללא backend
  ובלי חשיפת מייל אישי. (`opt.about.feedback` בשתי השפות.)
- **`ROADMAP-PRO.md`** — עודכן סטטוס: מה כבר בוצע ומה נשאר.

---

## v2.7.2 — מסך הסכמה חד-פעמי (דיסקליימר) בהפעלה ראשונה

**הרקע:** לקראת פרסום ציבורי, חשוב שהמשתמש יראה ויאשר את תנאי השימוש פעם
אחת — שהכלי לא רשמי, לשימוש אישי בלבד, וללא אחריות.

- **`popup.html`** — נוסף overlay `#disclaimerOverlay` עם הכותרת, שלוש נקודות
  (שימוש אישי בלבד / ToS באחריות המשתמש / AS IS) וכפתור הסכמה; CSS תואם RTL+dark
- **`popup.js`** — `maybeShowDisclaimer()` מציג את ה-overlay בפתיחה הראשונה עד
  שלוחצים "הבנתי", ושומר את ההסכמה ב-`chrome.storage.local` (`mhDisclaimerAccepted`)
- **`i18n.js`** — כל טקסטי הדיסקליימר בעברית ובאנגלית

---

## v2.7.1 — הרשאת debugger הפכה לאופציונלית (on-demand)

**הרקע:** הרשאת `debugger` הייתה ברשימת ההרשאות החובה, אבל היא בשימוש **רק**
עבור כלי האבחון האופציונליים של Zoom ("מחקר עמוק" ו"תיעוד בקשות רשת"). הרשאת
debugger חובה גוררת באנר מפחיד למשתמש ("Moodle Hoarder started debugging this
browser") ובדיקה מחמירה ב-Chrome Web Store — שניהם מיותרים לפיצ'ר ליבה.

- **`manifest.json`** — `debugger` הועברה מ-`permissions` ל-`optional_permissions`
- **`popup.js`** — חדש: `ensureDebuggerPermission()` מבקש את ההרשאה on-demand
  מתוך ה-user gesture של הלחיצה (במקום הבדיקה הישנה `if (!chrome.debugger)`),
  גם ל"מחקר עמוק" וגם לתיעוד הרשת; אם המשתמש מסרב — הודעה ברורה והפעולה מדולגת
- כל שאר התוסף (הורדות קורסים, קישורים, תמלילים, וידאו) לא נוגע ב-debugger כלל,
  אז המשתמש הרגיל לעולם לא יראה את הבאנר
