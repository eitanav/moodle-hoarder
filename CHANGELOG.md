...
## v1.35.0 — שינוי שם חכם ל-PDF לפי כותרת (ROADMAP #21)

**הבעיה:** חלק מקבצי ה-PDF במודל יורדים בשם גנרי וחסר משמעות — `1.pdf`, `unnamed.pdf`, `מסמך.pdf`, או `Microsoft Word - doc1.docx.pdf`. קשה למצוא אותם אחר כך ב-ZIP.

**התיקון:** מודול חדש `pdf-title.js` מחלץ את הכותרת מתוך נתוני ה-PDF עצמו — בלי pdf.js הכבד ובלי בינה מלאכותית. הוא קורא את ה-`/Title` מ-Info dictionary (מחרוזת literal או hex, כולל UTF-16BE לעברית) ואת `dc:title`/`pdf:Title` מ-XMP (UTF-8). אם הכותרת נמצאה והשם הנוכחי גנרי — הקובץ מקבל את שם הכותרת.

**עקרונות בטיחות:**
- **כבוי כברירת מחדל** (טוגל "שינוי שם חכם ל-PDF" בהגדרות). שינוי שם הוא שינוי גלוי, אז המשתמש בוחר להפעיל.
- **נוגע רק בשמות גנריים** — קובץ שכבר יש לו שם משמעותי (עברית או אנגלית) נשאר כמו שהוא.
- **best-effort מלא** — אם זה לא PDF, אין מטא-דאטה, הכותרת עדיין גנרית, או הקובץ ענק (>64MB) — נשאר השם המקורי, בלי קריסה.
- מסיר prefix של "Microsoft Word - " וכותרות-זבל שאינן כותרת אמיתית.

**איך לבחון:** להפעיל את הטוגל בהגדרות → להוריד קורס עם PDF ששמו גנרי אבל יש לו כותרת פנימית → לוודא שהקובץ ב-ZIP קיבל את שם הכותרת. קובץ עם שם תקין מראש לא אמור להשתנות. (יש 28 בדיקות יחידה ב-`test/pdf-title.test.js` — `node test/pdf-title.test.js`.)

זה הפיצ'ר האחרון מתוך 16 שהמשתמש סימן ב-ROADMAP. ה-manifest עודכן ל-`1.35.0`.
