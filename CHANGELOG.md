...
## v1.33.0 — שינוי שם PDF לפי הכותרת (ROADMAP #21)

**הבעיה לפני:** מודל מגיש לא מעט PDFים בשמות חסרי משמעות — `1.pdf`, `document.pdf`, `scan0001.pdf`, מחרוזות hash, או ה-fallback שלנו `resource_<id>.pdf`. אחרי הורדה של קורס שלם נשארים עשרות קבצים שאי אפשר לזהות בלי לפתוח כל אחד.

**הפתרון:** טוגל חדש בהגדרות (**"שינוי שם PDF לפי הכותרת"**, ברירת מחדל **כבוי**). כשהוא דלוק, כל PDF שיורד עם שם **גנרי** מקבל את השם מתוך כותרת ה-PDF עצמה (מטא-דאטה). קבצים עם שם משמעותי לא נוגעים בהם בכלל.

- **בלי בינה מלאכותית ובלי pdf.js** — קריאה ישירה של הבייטים. שתי מקורות לכותרת:
  1. ה-Info dictionary `/Title` — תומך ב-literal string `(...)` (כולל סוגריים מקוננים, escapes, octal, line-continuation) וב-hex string `<...>`.
  2. fallback ל-XMP `dc:title` אם אין Info/Title.
- **עברית עובדת** — מזהה UTF-16BE עם BOM (כך מקודדות כותרות עברית ב-PDF), UTF-16LE, UTF-8, ו-Latin1/PDFDocEncoding.
- **זיהוי "שם גנרי"** (`isGenericName`): מספרים בלבד, hash הקסדצימלי ארוך, `resource_<id>`, ומילון מילים גנריות (document/scan/image/copy/מסמך/סריקה/עותק...) עם או בלי מספר נלווה.
- **בטוח:** לעולם לא זורק ולא חוסם הורדה — בכל כשל משאיר את השם המקורי. כותרת מסוננת ב-`sanitizeFilename`. התנגשויות שמות נפתרות ע"י ה-`uniquePath` הקיים. לקבצים מעל 12MB סורקים רק 2MB מההתחלה ו-2MB מהסוף (שם יושבת המטא-דאטה) כדי לא לבזבז זיכרון.

**איך לבחון:** הגדרות → תוכן → הדלק "שינוי שם PDF לפי הכותרת" → סרוק קורס עם PDFים בשמות גנריים → הורד → בדוק שהקבצים בתוך ה-ZIP קיבלו שמות קריאים מתוך הכותרת. קבצים שכבר היה להם שם טוב נשארים כפי שהם.
