...
## v1.35.0 — קיצור שמות קטעים חכם (ROADMAP #17)

**הבעיה:** שמות הקטעים במודל לרוב ארוכים — `תרגול 1 — תכונות זורמים ולחץ פרק א'` — והם הופכים לשמות תיקיות ארוכים ב-ZIP וגם מעמיסים על הבורר. לפעמים מספיק החלק המוביל (`תרגול 1`).

**מה השתנה:** נוסף טוגל **"קיצור שמות קטעים"** בהגדרות (סקציית "מבנה ה-ZIP"). כשהוא דלוק, שם קטע נחתך בנקודת ההפרדה הראשונה (`—` / `–` / `-` / `:`) ונשארת רק התווית שלפניה. הקיצור משפיע גם על שמות התיקיות ב-ZIP וגם על התצוגה בבורר. הלוגיקה (`shortenSectionName` ב-popup.js) שמורה: היא מקצרת רק כשיש תוכן אמיתי משני צדי המפריד, ולא נוגעת בשמות בלי מפריד, במילים מקופפות (`Co-op`), או בכותרת שתתקצר למספר בודד (`01 - מבוא` נשאר שלם). ברירת מחדל **כבוי** כי הקיצור lossy — בחלק מהקורסים החלק החשוב נמצא דווקא אחרי המקף.

**איך לבחון:** הגדרות → "מבנה ה-ZIP" → להדליק "קיצור שמות קטעים". לסרוק קורס עם שמות קטעים מרובי-מקף → לוודא שהבורר מציג שמות מקוצרים, ושתיקיות ה-ZIP נקראות לפי השם המקוצר. לכבות → השמות המלאים חוזרים.
