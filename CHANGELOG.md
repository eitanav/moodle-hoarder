...
## v1.33.0 — בורר האיכות ('שאל אותי') חזר לנתיב ההורדה החדש

ב-1.32.0 עברנו את הורדת הווידאו ל-fetch-בתוך-הדף (ה-worker פותח טאב, תופס signed URL, מושך blob ושומר). בנתיב הזה ה-worker בחר תמיד את הרזולוציה הגבוהה ביותר, ולכן **'שאל אותי' התנהג כמו 'הטובה ביותר'** — הבורר נעלם בפועל (סומן בשינוי כ-"chooser TBD").

### מה השתנה

עכשיו כשבורר האיכות מוגדר ל**'שאל אותי'** והמשתמש לוחץ 🎥:

1. הפופאפ קודם **מאתר את הרזולוציות הזמינות** — פותח טאב נסתר לכל הקלטה, תופס את כל ה-signed URLs, וקורא את ה-WxH משם הקובץ (`resolveRecordingCandidates` + `summariseResolutions`, פונקציות שכבר היו בקוד).
2. **מודאל הבחירה** (`chooseQualityDialog`) קופץ עם רשימת הרזולוציות הייחודיות, וליד כל אחת "זמין ב-X מתוך Y הקלטות".
3. הבחירה (תווית `WxH`, למשל `1366x768`) מועברת ל-worker ברקע. ה-worker **תופס מחדש** signed URLs בזמן ההורדה (signed URLs פגים תוך ~שעתיים — תפיסה מחדש per-job עמידה יותר מלמחזר את מה שנתפס בשלב האיתור) ובוחר את ה-URL **התואם לתווית** — ואם אין התאמה מדויקת באותה הקלטה, את הרזולוציה **הקרובה ביותר**.

נתיבי 'הטובה ביותר' / 'הקטנה ביותר' לא נגעו — הם עדיין שולחים את ההקלטה ישירות ל-worker בלי שלב איתור מקדים.

### מגבלות שנשארו (ללא שינוי מ-1.32)

- **בלי תת-תיקייה** בנתיב ה-anchor download — הקובץ יורד ישירות ל-Downloads.
- כלל ה-Referer/DNR שנשאר מיותר (הוכח ב-1.31.3) — עדיין ממתין לניקוי אחרי אישור שההורדה עובדת.

### איך לבחון

עדכן ל-1.33.0 (update.bat → Reload) → סרוק קורס → סמן כמה הקלטות → בבורר האיכות בחר **"שאל אותי"** → 🎥. אמור: סטטוס "מאתר רזולוציות", ואז מודאל עם הרזולוציות הזמינות. בחר אחת → ההורדות נכנסות לתור ברקע באותה רזולוציה (או הקרובה ביותר אם להקלטה מסוימת אין אותה בדיוק). **לא נבדק על ידי המפתח — אנא אשר.**
