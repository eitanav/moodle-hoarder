...
## v2.4.0 — שינוי שם אוטומטי ל-PDF לפי הכותרת (ROADMAP #21)

- **שינוי שם אוטומטי ל-PDF** — קבצי PDF עם שם גנרי (`1.pdf`, `scan.pdf`, `document.pdf`, `unnamed.pdf`, תאריך וכו') מקבלים את הכותרת הפנימית (`/Title`) של ה-PDF כשם הקובץ, אם הכותרת משמעותית.
- **בלי pdf.js, בלי AI, בלי אינטרנט** — הכותרת נקראת ישירות מבייטים של ה-PDF: תמיכה ב-literal strings ו-hex strings, פענוח UTF-16BE (כותרות עבריות) ו-PDFDocEncoding/Latin-1. PDF שמסתיר את המטא-דאטה ב-object streams דחוסים פשוט שומר על שמו המקורי.
- **בטוח כברירת מחדל** — טוגל בהגדרות (`renamePdfByTitle`), כבוי כברירת מחדל. רץ רק על קבצים שהשם שלהם גנרי, רק אם הכותרת אינה גנרית בעצמה, וקבצים מעל 40MB לא נסרקים. כל כישלון נופל בשקט לשם המקורי.
- **לוג** — כשמשנים שם, השורה בלוג מציגה `✎ שונה שם PDF: "ישן" ← "חדש"`.
- מסיים את ה-SPRINT המסומן ב-ROADMAP-100 (#21 היה הפריט האחרון שנותר).
