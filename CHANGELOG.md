...
## v2.4.0 — Chunking + Resume לתמלול הקלטות ארוכות (Transcriber)

- **הבעיה:** תמלול של הקלטה בת שעות שנפל/בוטל באמצע אילץ להתחיל מאפס ולשרוף שוב את כל זמן ה-GPU. זו הייתה המגבלה האחרונה לפני תמלול מאסיבי של עשרות שעות.
- **התיקון:** הקלטות ארוכות מחולקות עכשיו למקטעים (windows) של אורך קבוע, כל מקטע מתומלל בנפרד ונשמר כ-checkpoint על הדיסק:
  - **`chunking.py`** חדש — `plan_chunks` (חלוקה לחלונות לא-חופפים), `slice_wav` (חיתוך מדויק של ה-WAV המוכן עם מודול `wave` של stdlib, בלי קריאת ffmpeg נוספת), ועזרי resume (signature/manifest, קריאה/כתיבה אטומית של תוצאות מקטע).
  - **`engine.py`** — נתיב chunked חדש: טוען את המודל פעם אחת, עובר מקטע-מקטע, ולכל מקטע שמסתיים כותב `chunk_NNNN.json` עם timestamps מוחלטים בתיקיית `.mh_resume_<שם>` ליד הפלט. ריצה חוזרת עם אותו קובץ/מודל/אורך-מקטע מדלגת על מקטעים שכבר הסתיימו וממשיכה מהמקום שנעצר. בסיום מלא תיקיית ה-resume נמחקת.
  - ה-manifest שומר חתימה (מקור/מודל/אורך-מקטע/משך); אם משהו לא תואם, ה-checkpoint הישן נמחק ומתחילים מחדש — אי אפשר לטעון בטעות תוצאות של ריצה אחרת.
- **ברירות מחדל:** CLI ו-GUI מחלקים אוטומטית הקלטות מעל 30 דקות (`--chunk-minutes 30`, שדה "מקטע (דק')" ב-GUI). הקלטות קצרות יותר רצות ב-pass יחיד בדיוק כמו קודם. `--chunk-minutes 0` מכבה לגמרי, `--no-resume` מתעלם מ-checkpoint קיים.
- **הערה:** החיתוך הוא בגבולות זמן קשיחים (עם VAD בכל מקטע); ייתכן חיתוך נדיר של מילה בגבול מקטע. overlap+dedup הוא שיפור עתידי אפשרי.
- **בדיקות:** `test_chunking.py` — חלוקה אחידה/לא-אחידה, חיתוך WAV ו-clamping מעבר לסוף, roundtrip של checkpoint, ושינוי signature לפי קלט.
