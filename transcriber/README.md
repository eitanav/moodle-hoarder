...
## חידוש תמלול שנקטע (resume)

מ-v2.4.0 התמלול עמיד לנפילות. כל סגמנט שמתומלל נשמר מיד לקובץ checkpoint בשם `<שם הקובץ>.transcribe-progress.jsonl` בתיקיית הפלט. אם החלון נסגר, ה-GPU נופל או החשמל נופל באמצע תמלול של קובץ ארוך — פשוט הרץ שוב את אותו קובץ. הכלי יזהה את ה-checkpoint, ידלג עם ffmpeg על מה שכבר תומלל, וימשיך מהסגמנט האחרון במקום להתחיל מאפס.

- ה-checkpoint נמחק אוטומטית ברגע שכל קבצי התמלול (TXT/SRT/VTT/JSON) נכתבו. קובץ `*.transcribe-progress.jsonl` שנשאר פירושו שהרצה קודמת לא הסתיימה.
- אם שינית/הורדת מחדש את קובץ המקור, או שינית מודל/שפה — ה-checkpoint נחשב לא רלוונטי והתמלול מתחיל מחדש.
- לכבות לגמרי: `--no-resume` ב-CLI. resume דורש את שלב ה-ffmpeg, ולכן מושבת אוטומטית עם `--no-preprocess-audio`.
...
2. לבדוק על הקלטה של שעה — ובאמצע לסגור את החלון ולהריץ שוב, כדי לוודא שה-resume ממשיך מאיפה שעצר.
