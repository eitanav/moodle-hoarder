...
## הקלטות ארוכות: חלוקה למקטעים + המשך אוטומטי (Resume)

הקלטות בנות שעות מחולקות אוטומטית למקטעים (windows) של אורך קבוע, וכל מקטע שמסתיים נשמר כ-checkpoint על הדיסק. אם הריצה נופלת או מבוטלת באמצע — ריצה חוזרת על אותו קובץ ממשיכה מהמקטע האחרון שהסתיים במקום להתחיל מאפס.

- ברירת המחדל: הקלטות מעל **30 דקות** מחולקות אוטומטית. קצרות יותר רצות ב-pass יחיד כמו קודם.
- ה-checkpoints נשמרים בתיקיית `.mh_resume_<שם-הקובץ>` ליד תיקיית הפלט, ונמחקים אוטומטית כשהתמלול המלא נכתב בהצלחה.
- אם משנים קובץ/מודל/אורך-מקטע — ה-checkpoint הישן נמחק ומתחילים מחדש (אי אפשר לערבב תוצאות של ריצות שונות).

```powershell
# מקטעים של 20 דקות במקום 30
python transcribe.py "C:\path\to\lecture.mp4" --chunk-minutes 20

# כיבוי החלוקה לגמרי (pass יחיד תמיד)
python transcribe.py "C:\path\to\lecture.mp4" --chunk-minutes 0

# התעלמות מ-checkpoint קיים ותמלול כל המקטעים מחדש
python transcribe.py "C:\path\to\lecture.mp4" --no-resume
```

ב-GUI יש שדה "מקטע (דק')" — `0` מכבה את החלוקה.

> הערה: החיתוך הוא בגבולות זמן קשיחים (עם VAD בכל מקטע). נדיר, אבל ייתכן חיתוך של מילה בגבול בין מקטעים; overlap+dedup הוא שיפור עתידי אפשרי.
...
2. לבדוק על הקלטה של שעה (אמורה להתחלק ל-2 מקטעים עם resume).
3. להוסיף ניקוי/סיכום עם LLM ו-JSON שמתאים ל"שאל את הקורס".
4. בהמשך: diarization וחיבור אוטומטי ל-recordings של התוסף.
