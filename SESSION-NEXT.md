...
## ✅ עודכן (v1.32.0) — תוקנה הורדת ה-Zoom: fetch→blob

הסאגה של ה-`.htm` נסגרה בקוד. v1.31.0 פיצח את **המנגנון** (Referer של דומיין החשבון), אבל ההורדה עדיין שמרה `.htm` כי **DNR `modifyHeaders` לא חל על `chrome.downloads.download`** (browser-initiated). הפתרון שיושם ב-v1.32.0: **`fetchRecordingBlob()`** מביא את ה-MP4 ב-`fetch` מהפופאפ (זו בקשת `xmlhttprequest` ש-DNR **כן** משנה → Referer תקין → וידאו אמיתי), ואז **`downloadBlobAndRevoke()`** שומר דרך `URL.createObjectURL`. כולל מד התקדמות בייטים והגנות מפני HTML/empty/expired.

**שלב ראשון בסשן הבא:** לשאול את המשתמש אם **🎥 הורד סרטונים** ב-v1.32.0 הוריד `.mp4` תקין שמתנגן (לא `.htm`).

- אם **כן** → 🎉 הורדת הוידאו עובדת מקצה לקצה. שקול לקדם ל-**2.0** (ראה למטה "על גרסה 2.0"), ואז לעבור ל-ROADMAP-100.
- אם **לא** → להריץ 🩺 דיאגנוסטיקה ולבדוק את `download-probe.withReferer`: אם הוא `video/mp4` אבל ההורדה עדיין נכשלת → לבדוק את הלוג של הפופאפ (שגיאת fetch / CORS / quota זיכרון). אם הוא HTML → ה-URL פג, צריך לסרוק מחדש סמוך ללחיצה.

**מגבלה ידועה לטיפול עתידי:** `fetchRecordingBlob` מחזיק את כל הקובץ בזיכרון. ל-~115MB עובד; לקבצים של כמה GB צריך streaming לדיסק (File System Access API — `showSaveFilePicker` → `WritableStream`, ואז לכתוב כל chunk מה-reader). זה גם יחזיר את היכולת "אפשר לסגור את הפופאפ".
...
### ✅ נפתר ב-v1.32.0 (היה: 1.31.0 עדיין מוריד .htm — Referer דרך DNR לא חל על chrome.downloads)

> **הסטטוס למטה הוא היסטורי** — האבחנה אומתה והפתרון (fetch→blob) יושם ב-v1.32.0. נשמר לתיעוד.
