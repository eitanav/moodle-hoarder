...
## מצב הורדת Zoom (v1.32.2) — קרוב מאוד, ממתין לאימות

**מה ידוע בוודאות (מ-download-probe):** ה-MP4 החתום של ssrweb מאשר את עצמו — `fetch` על ה-URL המלא מחזיר **206 video/mp4**, עם ובלי Referer ועם ובלי cookies. אז Referer/DNR מיותרים (הוסרו ב-1.32.2). הבעיה ההיסטורית (`.htm`) הייתה **`chrome.downloads.download` שמשבש את ה-URL החתום**.

**הנתיב הנוכחי (background.js `mh-download-rec`):** הפופאפ שולח הודעה ל-worker → ה-worker פותח את דף ההשמעה בטאב נסתר, תופס את ה-signed URL, ומריץ **בתוך הדף** `fetch(url, {credentials:'omit'})` → `blob` → `<a download>` click. תור סדרתי, ממתין לסיום דרך `chrome.downloads.onChanged`, ואז התראה. רץ ב-SW (שורד סגירת פופאפ).

**טרם נבדק ע"י המשתמש.** השלב הראשון בסשן הבא: לאשר שכפתור 🎥 מוריד MP4 שלם. אם נכשל — לבדוק את ה-Console של ה-service worker (`chrome://extensions` → service worker), והחשוד הבא הוא CORS על ה-fetch-בדף או מחזור-חיים של הטאב/SW. הראיה ש-`fetch` עצמו מחזיר וידאו אומרת שהכשל (אם יש) הוא בתפירה, לא במנגנון.

**סיכונים ידועים:** קבצים ענקיים (1.58GB) כ-blob בזיכרון — שביר; פתרון עתידי = streaming (File System Access / StreamSaver). 'שאל אותי' = 'הטובה ביותר' כרגע. אין תת-תיקייה.

## ✅ הורדת Zoom — נפתר ברמת ה-URL (v1.31.3)

ה-🩺 download-probe על ה-URL המלא: **206 video/mp4, עם ובלי Referer.** ה-URL החתום מאשר את עצמו. כל ה-`.htm` בעבר = URL חתוך (באג חיתוך 600/500 תווים שמחק את ה-Signature, תוקן ב-1.31.2). נתיב ההורדה האמיתי תופס URL מלא → אמור לעבוד. **ממתין לאישור שכפתור 🎥 מוריד MP4 שלם.** אם כן → לנקות (להסיר DNR + הרשאת declarativeNetRequest, מיותרים) ולדבר 2.0.
